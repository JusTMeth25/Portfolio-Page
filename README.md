# Portfolio — Lorenzo Melis

Portfolio personale di **Lorenzo Melis**, Junior Full-Stack Developer.
Sito statico a pagina unica: hero con scultura WebGL, tre progetti in evidenza,
profilo, percorso, competenze e contatti. Nessun backend, nessun database,
nessuna chiave API.

**Stack:** React 19 + TypeScript (strict) + Vite · Motion for React ·
Three.js / React Three Fiber · Lucide React · CSS con design token.
...
Tutto in **`src/data/profile.ts`**. Nessun testo personale è scritto nei
componenti.
...
Tutto in **`src/data/projects.ts`**. I componenti non contengono titoli, URL o
condizioni legate a un progetto specifico.
...
Il PDF attuale deriva dall'export Canva fornito (`Lorenzo_Melis_CV_Canva_Editable.pptx.pdf`, lasciato nella root come sorgente), con la sezione
*Selected Development Projects* riscritta sui tre progetti del portfolio
(Spotify Clone, Vinilshelf, EpiWeather) e i link "Repository" aggiornati ai
repository corrispondenti. Nell'operazione i font incorporati sono stati
ricostruiti: rispetto all'originale il testo del PDF è ora estraibile
...
`actions/configure-pages`, quindi **non devi impostare nulla a mano**.

Per attivarlo:

1. su GitHub: *Settings → Pages → Build and deployment → Source: **GitHub Actions***;
2. fai push sul branch `main` (il workflow parte anche manualmente da
   *Actions → Deploy su GitHub Pages → Run workflow*).

Un push successivo su `main` riesegue il workflow e ripubblica il sito.
**In questa sessione non è stato eseguito alcun push né alcuna pubblicazione.**
...
Vedi **`THIRD_PARTY_NOTICES.md`** per l'elenco completo, incluse la posizione
di React Bits (MIT + Commons Clause) e la scelta di non copiarne il codice, e la
...
**Non eseguito in questa sessione:** Lighthouse (non disponibile
nell'ambiente), quindi non riporto punteggi di Performance/Accessibility/SEO;
test dei tre repository dei progetti; qualsiasi push o pubblicazione.
